...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer The data is not separated that well into rows. The numbers are shown alongside each county, but not in a way that is easy to understand.**
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer The rows still need actual headers, there are still periods after the county names.**
...
**Task** Run the codeblock below. What else do we need to do? **Answer The periods still need to be removed from the left-most row.**
