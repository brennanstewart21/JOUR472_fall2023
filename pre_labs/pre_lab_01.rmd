...
In the space below, write the name of the business from the first row of the "name" column. **Answer A "name" column is not displayed**
...
**Answer The first line is grouping the classes together based on which term they are in. The summarise command is adding the courses together.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer The previous codeblock did not have the "arrange" command which sorts the terms in a descending order based on how many classes they had.**
...
**Answer This output displayed classes from the same term but sorted them by department. This was done using the "arrange" command.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer This codeblock is not limited to just one term, and it also includes the total seat counts for each term (not just the total classes). This was achieved by adding another line to the "summarise" command.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer This codeblock expands on the total seats, now including a mean and median for the total seats in each term. The top two terms have significantly higher class counts than the other terms, but both have means of 46.**
...
**Answer This codeblock now asks the program to include both the minimum and maximum number of seats for classes in each department. The data is sorted to descend from the highest to lowest number of total seats.**
...
1.  How does the mean number of classes change in the journalism department over the past 10 years?

2.  In 2022, which department had the lowest number of classes? Lowest number of seats?

3.  What is the maximum number of classes each department has had in the past 10 years? The minimum?
