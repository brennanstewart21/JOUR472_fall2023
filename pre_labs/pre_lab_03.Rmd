...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer 2010 and 2011 do not indicate whether the payments are a grant or a loan, but the rest of the years do.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer The majority of years listed in this row pre-date 2017. Only three of the seven items are dated from 2017 or more recent. When you scroll towards the end of this data set, the years seem to get more recent as the amount grows.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer They have been changed to lowercase and now have underscores between the words instead of spaces.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer There are 50 rows, each listed with a dupe_count value of two.**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer There are 17740 rows over the span of 100 pages.**
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Does that zip code make sense to you? 21285
