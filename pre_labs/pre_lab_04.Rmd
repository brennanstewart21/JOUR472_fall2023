...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer The grantees are not listed the same way. Similar grantees are not exactly similar, with some containing extra words or characters that others don't have.**
...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer Several grantees went from being all uppercase to all lowercase. or vice versa**
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer Some grantees, like Washington Adventist University, became lowercase once they were cleaned. Other grantees that contained abbreviations lost their periods after being cleaned.**
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer The changes do not vary substantially. Like the key_collision_merge function, many of the grantees have become lowercase or have lost their abbreviating periods.**
