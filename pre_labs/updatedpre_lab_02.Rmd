...
**Answer Its important to create a new variable in case you want to do any further analysis with your selected data. If you didn't create a new variable, you would have to go through the process of re-selecting what data you wanted to view every time.**
...
**Answer: The dates are now listed in a yyyy-mm-dd format rather than a mm/dd/yyyy format.**
...
**Answer In the new month column, the months from 2022 are listed together, followed by the months from 2021 when you move to the next page. It also only lists the first date from each month. They are sorted to descend from highest total amount to lowest.**
...
**Answer This code is creating a new column that shows the percentage of total votes that went to Wes Moore.**
...
**Answer The percentages are now shown so they descend from the highest value to the lowest value**
...
**Answer If you wanted to see which counties voted for Moore the least, this answer would be more useful. If you wanted to see where Moore got the highest percentage of votes, the previous codeblock is better.**
...
**Answer I used ChatGPT for this question. The question I submitted was, "What are two examples of tidyverse functions similar to str_to_upper? Describe what each of them do." The first that ChatGPT described was called str_to_lower, which is the opposite of str_to_upper as it converts all capitalized characters to lowercase. The second tidyverse function that ChatGPT described was str_trim, which removes trailing and leading spaces or tabs from characters. **
...
**Answer I think this mutate statement is looking at every address in the data set, then determining what state is listed with each address in order to make a new column that declares a state abbreviation for each one.**
...
**Answer This answer does not make sense to me, considering there were only 97877 items listed in the table. How would there be more total abbreviations for each state than total items?**
...
1.  Which departments at Maryland have had the most seats,
historically?

2.  In which counties does Wes Moore's voting percentage not exceed 50%?

3.  In which counties do the votes not exceed 150000 but still favor Wes Moore?
