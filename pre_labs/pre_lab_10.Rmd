...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer The data contains names of the zip codes that are listed, along with coordinates that represent geospatial data. Each column is a different zip code.**
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer The data contains the shape area and shape length for every zip code represented, as well as a fips code.**
...
**Task** Run the following code. Describe the output in the space below. **Answer A map is displayed with the zipcode data generated into it.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The output instructs me to set the cache to 'TRUE'. Once I do so, there is no output when I run it again.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The counties for the whole United States are displayed, along with data that is related to each one.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The county data from the previous task has been clipped down so it only shows the Maryland county data.**
...
**Task** Run the following code. Describe the output in the space below. **Answer A map is displayed that shows the shapes/outlines of every county in the state.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The two maps have been layered onto each other, showing the zip code boundaries within each county.**
...
**Task** Run the following code. Describe the output in the space below. **Answer A table with only two columns. One column is a list of zip codes, and the other contains a count on the notices of intent to foreclosure for each zip code in March of 2023.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The foreclosure data has been combined with the Maryland zip code data.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The output is a multi-colored map that changes color based on the number of foreclosures in a zip code.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The foreclosure counts are now arranged by county rather than by zip code.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The geospatial data for each county is shown at the end of each row.**
...
**Task** Run the following code. Describe the output in the space below. **Answer Alongside the county population data is the foreclosure data for each county.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The md_county_population data has new names above the columns and the dataset has been cleaned so the county names are all shown the same way.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The number of notices per county in Maryland have been combined with the population data.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The rate of foreclosure notice per population total is displayed in a descending order with the highest rates at the top of the dataset.**
...
**Task** Run the following code. Describe the output in the space below. **Answer A map is displayed with the rate of foreclosure per person in Maryland, grouped by county. The counties with the higher rates are in a lighter shade of blue.**
...
**Task** Run the following code. Describe the output in the space below. **Answer The map from above is altered so it displays different colors with more of a contrast.**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer Counties with the lowest rates are grouped mostly in the center of the state.**
