...
**Answer** <https://datawrapper.dwcdn.net/wKgkG/1/>
...
**Task** Look for "Hide regions without data" under Appearance, and click the slider icon to enable that feature. You should see a map zoomed into Maryland with some counties in various colors. **Answer The previous map showed every county in the United States, even though only the Maryland counties had data. Once the other regions were hidden, the map became only the state of Maryland and the data was much more easy to see.** Describe the changes from the previous map
...
**Task** Click the "Customize tooltips" button so it expands down. Change {{ fips_code }} to {{ county }} and {{ pctmoore }} to {{ FORMAT(pctmoore, "0.0%")}} **Answer Instead of displaying a FIPS code, the map now displays a percentage in 0.0% format. ** Describe the changes from the previous map
...
**Task** Click the "Proceed" button until you get to the "Publish & Embed" tab, then click "Publish Now". Copy the published URL (like you did for the chart above) and paste it below. **Answer https://datawrapper.dwcdn.net/1OJqa/1/ **
