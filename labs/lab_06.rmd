...
A1. From most to least: Carroll (448), Cecil (438), Allegany (418)
...
A2. Cecil County had the most calls with 55 in the month of August. 314 Grove Neck Rd, which is located in Cecil County, had the most calls out of all locations with a total of 15.
...
A3.21201 has the highest rate of calls per 1000 people. Upton has the most calls in that zip code with 201, followed by Penn North with 177 and Sandtown-Winchester with 174.

```{r}
baltimore_911_with_demographics |>
  group_by(zip_code, pct_under_18 > 75) |>
  summarize(count = n())

baltimore_911_with_demographics |>
  group_by(zip_code = 21201, neighborhood) |>
  summarize(count = n()) |>
  arrange(desc(count))
...
A4: Which Baltimore City zip code that has over 50% of its residents registered as homeowners has the most calls?

```{r}
baltimore_911_with_demographics |>
  group_by(zip_code, pct_homeowner > 50) |>
  summarize(count = n()) |>
  arrange(desc(count))
```
