...
```{r}
pg_calls_by_month <- pg_call |>
  group_by(month) |>
  summarize(count = n()) |>
  mutate(percentage = ((count)/1397)*100) |>
...
pg_calls_by_month |>
  ggplot() + 
  geom_bar(aes(x=reorder(month, percentage), weight=percentage)) + coord_flip() + 
  theme_minimal() +
  labs(
...
pg_calls_by_week |>
  ...
  labs(
    title="Overdose calls in Prince George's County spike mid-year",
    x = "Weeks (2022)",
    y = "Total calls",
    caption = "Source: Prince George's County EMS"
