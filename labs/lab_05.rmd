...
```{r}
baltco_911_overdose_calls <- read_csv('tabula-Baltimore County_Carey, Samantha log OD.csv', col_names = FALSE)
...
A1. I do not believe there are any days with no overdose calls at all, or at least not many days. You couldn't filter these results to show any dates with 0 calls, however, because only the reported calls are listed in the actual data set. Given the high frequency of the calls on a large majority of dates, it is unlikely that there are any days with zero calls.
...
A2. There appears to be a spike on the weekend days, with Sunday having the highest percentage and Saturday having the second-highest. Thursday and Friday have the lowest percentage of cases reported.
...
A3.4540 Silver Spring Rd has the most calls with 36. The original data does not include zip codes, only the first line of the street addresses. This could skew the counts if there were calls at different locations that contain the same street addresses. 
...
A4. Which month between 2022 and 2023 had the most overdose calls? OR Which month had the most calls on average over a two-year span? Regarding a story idea, this data could be used to determine what month of the year seems to have the most frequent cases of overdose.
